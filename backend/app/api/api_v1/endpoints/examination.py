...
async def get_ontology_info():
    ...
    try:
        ...
        logger.error(f"Error getting ontology info: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
@router.post("/import")
async def import_ontology_data(
    ...
    Args:
        file: CSV file with ontology data
    ...
    try:
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Ontology data imported successfully",
                "stats": stats
            }
        finally:
            # Clean up temp file
        ...
        logger.error(f"Error importing ontology data: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_complete_tree():
    """
    Get complete examination ontology tree structure.
    
    Returns:
        Nested tree: {level1: {level2: [methods]}}
    """
    try:
        await examination_kg_service.initialize()
        tree = await examination_kg_service.get_complete_tree()
...
@router.get("/graph/methods")
async def get_methods_for_part(
    level2: str = Query(..., description="Level 2 body part name")
):
    """
    Get examination methods for a specific level 2 body part.
    ...
    try:
        await examination_kg_service.initialize()
        methods = await examination_kg_service.get_methods_for_part(level2)
